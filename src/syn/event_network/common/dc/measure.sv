...
(
    ...
    output evn::link_delay_st_t      delay_status
);
    import evn::*;
    ...
    always_comb begin
        case (state)
            ZERO     : next = sample_error ? ERROR : average_upd         ? INITIAL  : ZERO;
            INITIAL  : next = sample_error ? ERROR : one_cycle_time == 0 ? ONE_CYCLE : INITIAL;
            ONE_CYCLE : next = sample_error ? ERROR : fine_time      == 0 ? FINE     : ONE_CYCLE;
            FINE     : next = sample_error ? ERROR : FINE;
            ERROR    : next = ZERO;
            default      : next = ERROR;
        endcase
    end
