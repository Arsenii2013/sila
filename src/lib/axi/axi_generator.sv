...
class axi_generator;
    ...
    task automatic read(input addr_t addr, output data_t data);
        item_t req_item = '{item_type:axi_transaction_pkg::READ, 
                                                addr:addr+base, data:data, resp_channel:resp_channel};
        item_t resp_item;
        ...
        forever begin
            ...
            if(resp_item.item_type == axi_transaction_pkg::READ_RESPONCE && resp_item.resp_channel == resp_channel) begin
                @(posedge clk_if.clk);
    ...
    task automatic verify(input addr_t addr, input data_t data);
        req_mailbox.put('{item_type:axi_transaction_pkg::VERIFY, addr:addr+base, data:data, resp_channel:resp_channel});
    endtask

    task automatic write(input addr_t addr, input data_t data);
        req_mailbox.put('{item_type:axi_transaction_pkg::WRITE, addr:addr+base, data:data, resp_channel:resp_channel});
    endtask

    task automatic write_verify(input addr_t addr, input data_t data);
        req_mailbox.put('{item_type:axi_transaction_pkg::WRITE_VERIFY, addr:addr+base, data:data, resp_channel:resp_channel});
    endtask
