...
typedef struct packed {
    ena_t                               ena;
    signal_generator_pkg::polarity_t    polarity;
...
class signal_generator_generator#(
    ...
    typedef signal_generator_pkg::polarity_t             polarity_t;
    ...
    task set_polarity(input int unsigned gen_n, input signal_generator_pkg::polarity_t polarity);
        if(polarity == signal_generator_pkg::POSITIVE)
    ...
    task dump();
        ...
        for(int i = 0; i < GEN_N; i++) begin
            ...
            $display("generator      : %d\n", i, 
                     "ena            : %b\n", rd_cfg.ena,
                     "polarity       : %s\n", rd_cfg.polarity.name,
                     "output source  : %s\n", rd_cfg.out_src.name,
                     "trigger source : %s\n", rd_cfg.trig_src.name,
                     "period         : %d\n", rd_period,
                     "delay          : %d\n", rd_delay,
                     "width          : %d\n", rd_width);
        end
